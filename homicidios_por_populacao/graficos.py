...
def densidade():
        histograma = (df_dados_brutos['Taxa homicidios']).plot.hist(density = True, bins = range(1, 11), figsize=(6,4))
        df_dados_brutos['Taxa homicidios'].plot.density(ax = histograma)
        histograma.set_xlabel('Taxa de homicidios')
        histograma.set_ylabel('Frêquecia (densidade)')
        plt.xlim(0, 15)
        plt.show()
...
def dispersao():
     ...
     plt.title('Gráfico de Dispersão: taxade homicidios vs população')
     plt.grid(True)
     plt.show()
